...
(() => {
  ...
  // глянцева, дещо кислотна пастель у дусі Frutiger Aero — аква, м'ята, небо, коралова, бузкова, сонячна
  const PALETTE = ['#3fc7c2', '#5fae63', '#4a90c9', '#ef9bb8', '#8f8fe0', '#ffb648'];
  ...
  function buildStars() {
    ...
    for (let i = 0; i < count; i++) {
      ...
      s.className = 'star';
      const size = (Math.random() * 22 + 6).toFixed(0) + 'px';
  ...
  // Великі фото з телефону (3-8 МБ) легко переповнюють localStorage (ліміт ~5-10МБ)
  // і роблять збереження непомітно важким. Стискаємо/зменшуємо кожне зображення
  // до розумного розміру ще до того, як воно потрапляє у сховище.
  function resizeImageFile(file, maxDim = 900, quality = 0.85) {
    return new Promise((resolve, reject) => {
      ...
      img.onerror = () => { URL.revokeObjectURL(objectUrl); reject(new Error('не вдалось прочитати зображення')); };
      img.src = objectUrl;
    });
  }
  ...
  async function handleFileInput(e) {
    ...
    $('#fileName').textContent = 'стискаю зображення…';
    try {
      pendingCoverData = await resizeImageFile(file);
      $('#fileName').textContent = file.name;
      refreshVisual();
    } catch (err) {
      console.error('Не вдалось обробити зображення', err);
      $('#fileName').textContent = 'не вдалось завантажити — спробуй інше фото';
      showToast('Це зображення не вдалось обробити');
    }
  }

  function handleSave() {
    try {
      if (!currentEdit) return; // модалка не відкрита належним чином — тихо виходимо
      ...
      closeModal();
      if (!ok) showToast('Річ на поличці, але не влізла у сховище браузера — спробуй фото менше вагою');
    } catch (err) {
      // ніколи не мовчимо: якщо щось пішло не так, людина побачить це, а не "нічого не відбулось"
      console.error('Помилка при збереженні', err);
      showToast('Щось пішло не так при збереженні: ' + err.message);
    }
  }
  ...
  // якщо десь у застосунку станеться неочікувана помилка — показуємо це,
  // а не залишаємо людину дивитись на кнопку, що "нічого не робить"
  window.addEventListener('error', (e) => {
    console.error('Неочікувана помилка:', e.error || e.message);
    showToast('Сталась несподівана помилка — спробуй ще раз');
  });
